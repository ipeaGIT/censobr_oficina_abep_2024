...
Talvez os conjuntos de dados mais comumente usados dos censos brasileiros sejam os microdados de pessoas e domicílios. No entanto, o IBGE também disponibiliza dados muito ricos sobre as características populacionais e ambientais agregados ao nível de setores censitários. Neste capítulo, mostramos como usar o pacote **{censobr}** para acessar facilmente os dados em nível de setor censitário usando a função `read_tracts()`.
...
### Examplo 1: Distribuição espacial da renda em 2010

Neste primeiro exemplo, criaremos um mapa da distribuição espacial da renda média per capita. A informação sobre o número total de residentes em cada setor censitário está disponível na tabela com cojunto de variáveis do bloco `"Basico"`, na variável `"V002"`. Já a informação sobre renda total do setor censiário pode ser encontradas na  tabela do bloco `"DomicilioRenda"`, na variável `"V003"`.
...
tracts_df10 <- tracts_df10 |> mutate(income_pc = V003 / V002)   # <5>
...
Agora a gente pode fazer um merge da tablea de população com os dados espaciais, calcular a área em Km<sup>2</sup> e a densidade demográfica de cada setor, e criar o mapa.
